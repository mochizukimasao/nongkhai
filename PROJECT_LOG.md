...
### [2025-11-25] 箇条書き先頭の記号を大きめの丸に変更（表示のみ）

#### 1. 問題の概要
- プレーンテキストは維持したまま、ハイライト層では`- `などの行頭記号をより見やすい大きめの丸にしたい

#### 2. 実施した修正
- `js/app.js`: `updateHighlights()`内で行頭が`-`または`*`の行を検出し、表示時のみ `&bull;` を含む`<span class="md-bullet-dot">` に置き換える処理を追加（テキスト内容自体は変更なし）
- `css/style.css`: `.md-bullet`/`.md-bullet-dot`スタイルを追加し、丸のサイズや余白を調整

#### 3. 検証結果
- ⏳ ブラウザでの表示確認待ち（コピーすると従来どおりハイフンのままか要確認）

#### 4. 次のステップ
- 箇条書きを入力し、見た目の丸が期待どおりか確認
- プレーンテキストのコピペで`-`のまま維持されることを再チェック
